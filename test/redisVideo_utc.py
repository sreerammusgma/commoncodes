...
class TestStringMethods(unittest.TestCase):
	...
	def test_3(self):# checking destination
		try:
			result = False
			ob1=redisVideo(destination="/home/sreeram/Vids/redis_videoxyz.avi") 
		except TypeError:
			result = True
	...
	def test_4(self): # checking key_prefix
		try:
			result = False
			ob1=redisVideo(destination="/home/sreeram/Videos/redis_videoxyz.avi",key_prefix=6379) 
		except TypeError:
			result = True
			self.assertTrue(result)
	...
	def test_5(self): # checking IP
		try:
			result = False
			ob1=redisVideo(destination="/home/sreeram/Videos/redis_videoxyz.avi",key_prefix="Grosjean", IP=1234 ) #, port=6379, password='', wait_time=30, skip_frames=0,log_obj = logging.getLogger("test_logger.log"))
		except TypeError:
			result = True
			self.assertTrue(result)
	...
	def test_6(self): # checking port 
		try:
			result = False
			ob1=folderVideo(destination,port="localhost")
		except TypeError:
			result = True
			self.assertTrue(result)
	...
	def test_7(self): # checking password
		try:
			result = False
			ob1=redisVideo(password=6379, wait_time=30, skip_frames=0,log_obj = logging.getLogger("test_logger.log"))
	...
	def test_8(self): # checking wait_time 
		try:
			result = False
	...
	def test_9(self): # checking skip_frames
		try:
			result = False
			ob1=redisVideo(destination="/home/sreeram/Videos/redis_videoxyz.avi",key_prefix="Grosjean", IP="localhost", port=6379, password='', wait_time='30', skip_frames='0',log_obj = logging.getLogger("test_logger.log"))
		except TypeError:
