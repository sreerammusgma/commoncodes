...
class TestStringMethods(unittest.TestCase):
	...
	def test_1(self): # test if the video is stored in the destination 
		self.assertTrue(os.path.exists(destination))
		

	def test_2(self): # test if the frames can be captured from the video stored in the destination
	...
	def test_4(self): # checking camera number
		try:
			result = False
			ob1=cameraVideo(camera='s')
		except TypeError:
			result = True
			self.assertTrue(result)

	def test_5(self): # checking destination
		try:
			result = False
			ob1=cameraVideo(camera='1',destination= "/home/sr/videopqr.avi")
	except TypeError:
		result = True
		self.assertTrue(result)

	def test_6(self): # checking skip_frames
		try:
			result = False
			ob1=cameraVideo(camera='1',destination= "/home/sr/videopqr.avi",skip_frames='q')
		except TypeError:
			result = True
			self.assertTrue(result)
